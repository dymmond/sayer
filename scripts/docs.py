...
def _snapshot(paths: list[Path], ignored: tuple[Path, ...] = ()) -> dict[str, int]:
    state: dict[str, int] = {}

    for path in paths:
        if not path.exists() or any(_is_under(path, ignored_path) for ignored_path in ignored):
            continue
        ...
        for candidate in sorted(path.rglob("*")):
            if not candidate.is_file():
                continue
            if any(_is_under(candidate, ignored_path) for ignored_path in ignored):
                continue
...
def _prepare() -> list[Path]:
    generated = prepare_docs_tree(
        source_dir=SOURCE_DOCS_DIR,
        output_dir=GENERATED_DOCS_DIR,
        excluded_paths=(GENERATED_DOCS_DIR,),
    )
...
def _watch_sources(stop_event: threading.Event, interval: float = 0.5) -> None:
    ...
    ignored = (GENERATED_DOCS_DIR,)
    previous = _snapshot(watch_paths, ignored=ignored)

    while not stop_event.wait(interval):
        current = _snapshot(watch_paths, ignored=ignored)
        if current == previous:
            continue
        previous = current
        try:
