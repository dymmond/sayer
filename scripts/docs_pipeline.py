...
def _is_excluded(path: Path, excluded_paths: set[Path]) -> bool:
    ...
    return any(resolved == candidate or candidate in resolved.parents for candidate in excluded_paths)
...
def prepare_docs_tree(
    ...
    *,
    ...
    for source_file in sorted(source_dir.rglob("*")):
        if _is_excluded(source_file, excluded):
            continue
